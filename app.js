...
const blankState=editable().map(e=>e.tagName==='INPUT'?e.value:e.innerText);
function restore(values){editable().forEach((e,i)=>{if(typeof values[i]!=='string')return;if(e.tagName==='INPUT')e.value=values[i];else {e.innerText=values[i];}});calc();}
function persist(){try{localStorage.setItem(storageKey,JSON.stringify({version:1,values:editable().map(e=>e.tagName==='INPUT'?e.value:e.innerText)}));statusNode.textContent='이 브라우저에 자동 저장됨 · '+new Date().toLocaleTimeString('ko-KR');}catch(e){statusNode.textContent='자동 저장 불가 · 창을 닫기 전에 PDF/JPG를 저장하세요.';}}
...
function cellText(cell){return cell.querySelector('input')?.value??cell.innerText;}
...
[...document.querySelector('#roster tbody').rows].forEach((row,i)=>{let yy=y+60+i*25;box(32,yy,1536,25,i%2?'#f5f9fa':'#ffffff');[...row.cells].forEach((cell,j)=>{let val=cellText(cell);if(j>=17&&j<25&&val!=='')val=Number(String(val).replaceAll(',','')).toLocaleString('ko-KR');text(val,xs[j],yy,ws[j],25,11,ink,false,j>=17&&j<25?'right':'center');});box(32,yy+24.5,1536,.5,'#dce7e9');});
